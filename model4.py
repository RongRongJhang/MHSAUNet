...
class Denoiser(nn.Module):
    def __init__(self, num_filters, input_channels=1, kernel_size=3, activation='relu'):
        super(Denoiser, self).__init__()
        self.conv1 = nn.Conv2d(input_channels, num_filters, kernel_size=kernel_size, padding=1)
        ...
        # 調整 output_layer 的輸入通道數為 2，因為輸出需要匹配拼接後的 cb 和 cr
        self.output_layer = nn.Conv2d(input_channels, input_channels, kernel_size=kernel_size, padding=1)
        self.res_layer = nn.Conv2d(num_filters, input_channels, kernel_size=kernel_size, padding=1)
...
class MHSAUNet(nn.Module):
    def __init__(self, filters=32):
        ...
        # 只有一個 Denoiser，輸入通道數為 2（cb 和 cr 拼接）
        self.denoiser_cbcr = Denoiser(num_filters=filters // 2, input_channels=2)
        
        self.lum_pool = nn.MaxPool2d(8)
        self.lum_mhsa = SELayer(channel=filters, reduction=16)
